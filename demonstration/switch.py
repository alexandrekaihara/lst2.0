...
class Switch(Node): 
    ...
    def __collectFlows(self) -> None:
        try:
            ...
            subprocess.run(f"docker exec {self.getNodeName()} sudo /TCPDUMP_and_CICFlowMeter-master/capture_interface_pcap.sh {self.getNodeName()} /TCPDUMP_and_CICFlowMeter-master/collecteddata 2> /dev/null &", shell=True)
        except Exception as ex:
